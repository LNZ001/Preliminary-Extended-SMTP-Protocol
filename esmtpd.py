#! /usr/bin/python3.6
# changed by lnz.(2019-2-20)
# 没有选择继承而是直接修改的原因是需要修改SMTPServer用到的SMTPChannel类
...
program = sys.argv[0]
__version__ = 'Python SMTP proxy version 0.3+ (changed 2019-2-21)'
...
class SMTPChannel(asynchat.async_chat):
    ...
    # properties for backwards-compatibility 向后兼容性
    @property
    def __server(self):
    ...
    # Overrides base class for convenience. 为方便起见重写基类。
    def push(self, msg):
    ...
    def found_terminator(self):
        ...
        if self.smtp_state == self.COMMAND:
            ...
            if self.data_size_limit and self.num_bytes > self.data_size_limit:
                ...
                self.num_bytes = 0
                return
            # Remove extraneous carriage returns and de-transparency according 移除外部回车
    ...
    def smtp_USER(self, arg):
        if not arg:
            self.push('599 Syntax: USER username')
            return
        # See issue #21783 for a discussion of this behavior.
        if not self.regi:
            self.push('599 Duplicate REGI')
            return
        # 读取json数据库，检查auth账户
        with open('./auth.json','r') as load_f:
            self.auth = json.load(load_f)
            # print(self.auth)
        if arg in self.auth.keys():
            # 账户名已使用
            self.push('599')
        self.username = arg
        self.canregi = True
    ...
    def smtp_LOGINUSER(self, arg):
        self.user = arg
        self.push('250 OK')


    def smtp_LOGINPASSWD(self, arg):
        with open('./auth.json','r') as load_f:
            self.auth = json.load(load_f)
            if self.auth.get(self.user, None) == arg:
    ...
    def smtp_MAIL(self, arg):
        # 发送之前需要问候语，在这里可以进而检查登录。
        # 该代码并没有检查msg中的from和to是否不真实，实际上不一致会被主流smtp服务器视为垃圾邮件拒收。
        if not self.seen_greeting:
            self.push('503 Error: send HELO first')
            return
        # owner为当前登录的账户名称
        # if not arg == self.owner:
        #     self.push('599 FAIL')
        ...
        address, params = self._getaddr(arg)
        # owner为当前登录的账户名称
        if not address == self.owner:
            self.push('599 INVALID')
